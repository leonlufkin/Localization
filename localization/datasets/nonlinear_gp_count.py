...
class NonlinearGPCountDataset(Dataset):
  ...
  def __getitem__(self, index: int | slice) -> ExemplarType:
    ...
    signs = jnp.sign(exemplars)
    signs_ = jnp.concatenate([signs, signs[:,:1]], axis=1)
    labels = (jnp.diff(signs_, axis=1) != 0).sum(axis=1).astype(jnp.float32) / self.num_dimensions
    # subsample
...
if __name__ =="__main__":
    ...
    dataset = NonlinearGPCountDataset(key=key, xi=xi, gain=gain, num_dimensions=40, num_exemplars=10000)
    x, y = dataset[:10000]
    xx = (x.T @ x) / len(x)
    ...
    ax2.plot(x[0])
    ax3.hist(y, bins=jnp.arange(41) / 40)
    fig.savefig(f'../datasets/nlgp_count/covariance_{xi}_{gain}.png')
    plt.close()
